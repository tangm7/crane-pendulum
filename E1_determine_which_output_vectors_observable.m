...
qdot = [diff(x(t),t); -diff(q1(t),t); -diff(q2(t),t) ] % long time spent with error here, it should be a minus sign as shown.
...
[eigVectors eigValues] = eig(A_linear);

simplify(eigVectors)

simplify(eigValues)

simplify(eig(A_linear))

simplify(eigVectors(3)-eigVectors(4))


lambda = simplify(   eig(A_linear)   )
...
%%%%%%%%% find basis of three key vectors, eigenvalue 1
A_linear_lambda1 = A_linear - lambda1*eye(6);
SpecialThree1 = [  A_linear_lambda1(:,3)  A_linear_lambda1(:,5)  B_linear  ];
basis_lambda1 = simplify(   colspace(SpecialThree1)   )



%%%%%%%%% find basis of three key vectors, eigenvalue 2
A_linear_lambda2 = A_linear - lambda2*eye(6);
SpecialThree2 = [  A_linear_lambda2(:,3)  A_linear_lambda2(:,5)  B_linear  ];
basis_lambda2 = simplify(   colspace(SpecialThree2)   )


%%%%%%%%% find basis of three key vectors, eigenvalue 3
A_linear_lambda3 = A_linear - lambda3*eye(6);
SpecialThree3 = [  A_linear_lambda3(:,3)  A_linear_lambda3(:,5)  B_linear  ];
basis_lambda3 = simplify(   colspace(SpecialThree3)   )
...
%%%%%%%%%%%%%%%%%%%% use determinants
ConcatMatrix1 = [  A_linear_lambda1(:,2:end)  B_linear  ];
simplify(det(ConcatMatrix1))

ConcatMatrix2 = [  A_linear_lambda2(:,2:end)  B_linear  ];
simplify(det(ConcatMatrix1))


ConcatMatrix3 = [  A_linear_lambda3(:,2:end)  B_linear  ];
simplify(det(ConcatMatrix1))

display("conclusion: the system is not controllable if gravity is zero. Otherwise, it is controllable.")



%%% NOTE: CHANGED FROM  A_linear - lambda1*eye(6)  TO lambda1*eye(6) - A_linear
%%%%%%%%%%%%%%%%%%%% use determinants (fix mistake)
% det(   [A_linear_lambda1   B_linear; zeros(1,7)]   )


%%%%%%%%% find basis of three key vectors, eigenvalue 1
A_linear_lambda1 = lambda1*eye(6) - A_linear ;
SpecialThree1 = [  A_linear_lambda1  B_linear  ];
basis_lambda1 = simplify(   colspace(SpecialThree1)   )



%%%%%%%%% find basis of three key vectors, eigenvalue 2
A_linear_lambda2 = lambda2*eye(6) - A_linear ;
SpecialThree2 = [ A_linear_lambda2  B_linear  ];
basis_lambda2 = simplify(   colspace(SpecialThree2)   )


%%%%%%%%% find basis of three key vectors, eigenvalue 3
A_linear_lambda3 = lambda3*eye(6) - A_linear;
SpecialThree3 = [  A_linear_lambda3  B_linear  ];
basis_lambda3 = simplify(   colspace(SpecialThree3)   )


%%%%%%%%%%%%%%%%%%%%%%%%%%%% Matrix is 6x7, rank is equal to row rank (For an r x c matrix, If r is less than c, then the maximum rank of the matrix is r.)
%%%%%%%%%%%%%%%%%%%%%%%%%%%% we can grab the symbolic terms that matter only in a 3x3 matrix

%%%%%% first eigenvalue
threebythreematrix1 = [   SpecialThree1(2,3)   SpecialThree1(2,5)   B_linear(2);
                        SpecialThree1(4,3)   SpecialThree1(4,5)   B_linear(4);
                        SpecialThree1(6,3)   SpecialThree1(6,5)   B_linear(6);   ]

simplify(det(threebythreematrix1))

%%%%% second eigenvalue


threebythreematrix2 = [   SpecialThree2(2,3)   SpecialThree2(2,5)   B_linear(2);
                        SpecialThree2(4,3)   SpecialThree2(4,5)   B_linear(4);
                        SpecialThree2(6,3)   SpecialThree2(6,5)   B_linear(6);   ]

simplify(det(threebythreematrix2))
...
display(" Conclusion: so the determinant equals g^2/(M*l1*l2) (no matter what eigenvalue, hence the system is controllable unless gravity equals 0")



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% skipping LQR stuff for now (part D)
...
C_xonly = [1 0 0 0 0 0];
rank([C_xonly; C_xonly*A_linear; C_xonly*A_linear^2; C_xonly*A_linear^3; C_xonly*A_linear^4; C_xonly*A_linear^5;])
display("rank equals 6=n, so when x(t) is output vector, full state is observable!!!!!!!!")



%%% q1(t) q2(t) is output vector
C_q1_q2 = [0 0   1 0   1 0];
rank([C_q1_q2; C_q1_q2*A_linear; C_q1_q2*A_linear^2; C_q1_q2*A_linear^3; C_q1_q2*A_linear^4; C_q1_q2*A_linear^5;])
display("rank equals 4!=n, so when q1(t) q2(t) is output vector, not observable.")


%%% x(t) theta2(t) is output vector
C_x_q2 = [1 0   0 0   1 0];
rank([C_x_q2; C_x_q2*A_linear; C_x_q2*A_linear^2; C_x_q2*A_linear^3; C_x_q2*A_linear^4; C_x_q2*A_linear^5;])
display("rank equals 6=n, so when x(t) theta2(t) is output vector, full state is observable.")


%%% x(t) theta1(t) theta2(t) is output vector
C_x_q1_q2 = [1 0   1 0   1 0];
rank([C_x_q1_q2; C_x_q1_q2*A_linear; C_x_q1_q2*A_linear^2; C_x_q1_q2*A_linear^3; C_x_q1_q2*A_linear^4; C_x_q1_q2*A_linear^5;])
display("rank equals 6=n, so when x(t) theta1(t) theta2(t) is output vector, full state is observable.")
